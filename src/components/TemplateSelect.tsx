  ...
  return (
    <Listbox value={selected as any} onChange={(template: { code: string; label: string }) => onChange(template?.code)} disabled={disabled}>
      <div className="relative mt-2">
        <Listbox.Button className={`w-full rounded-lg border border-gray-300 px-3 py-2 text-sm sm:text-base bg-white text-[#5d5e60] focus:outline-none focus:ring-2 focus:ring-[#5d5e60] ${disabled ? "opacity-50" : ""}`}>
          ...
          <span className="absolute inset-y-0 right-0 flex items-center pr-2 pointer-events-none">
            <ChevronUpDownIcon className="h-5 w-5 text-[#5d5e60]" aria-hidden="true" />
          </span>
        </Listbox.Button>
            ...
            <Listbox.Option
              ...
              className={({ active, selected }: { active: boolean; selected: boolean }) =>
                `cursor-pointer select-none relative py-2 pl-10 pr-4 transition-colors ${
                  active ? "bg-[#5d5e60] text-white" : "text-[#5d5e60]"
                ...
                <>
                  ...
                  {selected ? (
                    <span className="absolute inset-y-0 left-0 flex items-center pl-3">
                      <CheckIcon className={`h-5 w-5 ${active ? "text-white" : "text-[#5d5e60]"}`} aria-hidden="true" />
                    </span>
                  ) : null}
                </>
              )}
